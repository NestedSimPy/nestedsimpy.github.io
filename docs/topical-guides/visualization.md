...
simulations launched at each **trigger event**. It works post-hoc, so the same
...
`visualize_inner` plots the branches launched at one trigger event. The time
axis is **relative to the trigger point** (the trigger fires at 0), so the branches line
...
om.visualize_inner(trigger_id=0, relative_start=0.0)  # only the branched future
```
...
| Argument | Type | Default | Meaning |
| --- | --- | --- | --- |
| `trigger_id` | `int` | — (required) | Which trigger event to plot (one of `om.trigger_ids`). |
...
| `relative_start`, `relative_end` | `float` | `None` | Clip the *relative* time axis (trigger point = 0); `relative_start=0.0` hides the outer lead-in, negative values show that much history before the trigger point. |
| `show_outer_context` | `bool` | `True` | Draw the outer segment before the trigger point as grey context. It meets the inner traces exactly at time 0, where the inner state equals the outer state. |
...
`metric="state_num_customers_in_queue"`), and a name the model recorded
with `env.record` works too — `metric="cost"` plots the recorded series
when no state column matches. User-defined custom metrics are
