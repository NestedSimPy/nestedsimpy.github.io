...
demand an order decision is made. The plain version always follows an
order-up-to rule. The nested version hands the decision to the engine:
at every review, each candidate order quantity is tried in inner
...
The policy function is identical in both files — state in, one complete
decision out. What changes is the decision line: the plain version calls
`base_policy(state)`, the nested version writes

```python
order = yield from env.decide(base_policy, state)
```

and declares the candidates with
`set_inner_actions(ACTIONS, metric="cost", outer_run_mode="rollout")`.
No triggering configuration is needed — the engine branches on the
event `decide` publishes. One `env.record("cost", period_cost)` line
exposes each period's cost so branches can be scored, and the run
directory gains a `rollout/` folder with one CSV per zoom level
(per-action scores, executed picks, one row per inner simulation, and
every decision inside every branch).
