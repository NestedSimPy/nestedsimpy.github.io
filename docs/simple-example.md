...
`export_inner_event_log(trigger_id, inner_id)` returns the event log of one
inner simulation. Below is the inner simulation with replication number k = 0
at trigger event 16, preceded by a few outer rows for context. The
**Simulation source** column says which part of the run each row belongs to:
`outer` before the trigger point, `inner` after it. The `(srv)` prefix in the
state columns is the server's `nested_id` (scroll it):
...
Adding this code generates two extra prediction-table columns,
`inner_user_wait_mean` and `inner_user_wait_std` (first rows shown):
...
<div class="ns-dataset-scroll">
<table class="ns-dataset-table">
<thead><tr><th>Customer</th><th>Mean inner wait</th><th>inner_user_wait_mean</th><th>inner_user_wait_std</th></tr></thead>
<tbody>
